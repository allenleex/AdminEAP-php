...
class ControllerCommand extends ServiceBase
{
    ...
    public function createEntity($info, $flag=true, $bundlename=null)
    {
        ...
        $engineType = $info->getEngine();
        $isBinary = $info->getIsBinary();
        $structure = (int)$info->getStructure();
        ...
        //从源表读取数据构成基础表
        $metadata = $this->get('db.clonebase')->getMetadata(true);
        ...
        $infoAttribute = $this->get('db.model_attribute')->findBy(array('model_id'=>$info->getId()), array('sort'=>'asc'), 100);

        $newinfo = array();//$this->get('db.model_fields')->findBy(array('plan_id'=>$info->getModelsplan(),'useCache'=>0), array('sort'=>'asc'));

        if($newinfo){
            foreach($newinfo['rows'] as $v){
                array_push($infoAttribute['rows'],$v);
            }
        }
        ...
        //加字段
        if(isset($infoAttribute['rows']))
        {
            foreach($infoAttribute['rows'] as $item)
            {
                $field    = $item->getField();
        ...
        if((bool)$structure)
        {
            ...
            //增加状态字段
            if(!isset($metadata->fieldNames['status']))
                $classMetadata->addField('status'
                    ,'boolean'
                    ,array('options'=>array('comment'=>'是否启用', 'unsigned'=>false), 'nullable'=>false));
        }
        ...
        if(!isset($metadata->fieldNames['identifier']))
            $classMetadata->addField('identifier'
                ,'string'
                ,array('options'=>array('comment'=>'唯一标识', 'unsigned'=>false), 'nullable'=>false, 'length'=>50));
        ...
        if(method_exists($info, 'getMode')&&$info->getMode()!=1&&$flag)
        {
            $bundle = $this->get('core.common')->getUserBundle();
            $bundle = strtolower(str_replace('Bundle', '', $bundle));

            //执行数据库动作
            $schemaTool = new SchemaTool($this->get('doctrine')->getManager($bundle));
...
str;
        ...
        //增加Action后缀
        if (!preg_match('/Action$/', $name)) $name .= "Action";
...
str;
    ...
    public function createService($bundle, $data, $userName=null)
    {
        ...
        $serviceName = isset($data['service'])?$data['service']:'';
...
str;
        ...
        $phpFilePath = $bundlePath."DB".DIRECTORY_SEPARATOR.$name.".php";
        ...
        if($serviceName)
        {
            ...
            {
                $dbInfo['services'][$serviceName]['class'] = $bundleNamespace."\\DB\\".$name;
    ...
    public function createViews(array $data, $clonetpl='')
    {
        ...
        //$viewPath = $bundlePath.'Resources'.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR;
        //$viewPath .= $this->generator->parseTemplatePath($template);
        ...
        $filesystem = new Filesystem();
        $clonetpl = $clonetpl?$clonetpl:'controller/Template.html.twig.twig';
