...
//'use strict';
    ...
    CommonTable.prototype.initTable = function (tableId, queryId, searchDiv) {
        ...
        var that = this;
        var columns = [];
        for (var i = 0; i < this.data.columnList.length; i++) {
            var column = this.data.columnList[i];
            var obj = {};
            obj["data"] = column.key;
        ...
        var allowPaging = this.data.pageSize;
        var rowId = "id";//this.data.key;
        this.table = $('#' + tableId).DataTable($.extend({
            ...
            "language": { // 中文支持
                "sUrl": "/bundles/core/common/json/zh_CN.json"
            },
            "displayLength": that.data.pageSize,// 每页记录条数，默认为10
            "serverSide": true,
            "ajaxDataProp": "data",
            "ajaxSource": basePath + "/"+listurl,
...
//刷新当前页面，并定位到行
    CommonTable.prototype.reloadRowData = function (rowId) {
        ...
        var pageIndex = pageInfo == null ? "first" : pageInfo.pageIndex - 1;
...
// isCondition默认为true;likeOption默认false 即不拼接%
    ...
    CommonTable.prototype.getServerData = function (pageInfo, tableId) {
        ...
        ajaxPost(basePath + "/"+listurl, reqParam, function (result, status) {
            retData = result;
        });
        var start = 0;
        if (pageInfo) {
        ...
        var columns = retData.columnList;
...
//匹配出[name]类似的字符串，并替换真实的值
    ...
    CommonTable.prototype.fillDataTable = function (sSource, aoData, fnCallback, oSettings) {
        ...
        if (this.loaded) {// 换页
            ...
            pageInfo.pageIndex = map.iDisplayStart % map.iDisplayLength == 0 ? map.iDisplayStart / map.iDisplayLength + 1
        ...
        obj["iTotalRecords"] = result.pageCount;
        obj["iTotalDisplayRecords"] = result.pageCount;
        fnCallback(obj);
        //序号排序
